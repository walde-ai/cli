...
export class CommandBriefCommentAdd {
  constructor(
  ...
  async execute(options: CommentAddOptions): Promise<void> {
    ...
    this.presenter.startOperation('Adding comment...');

    try {
      const result = await walde
        .brief({ id: options.briefId })
        .addComment({ content: options.content, author })
        .resolve();
