...
export class CommandBriefFail {
  ...
  async execute(options: FailOptions): Promise<void> {
    ...
    this.presenter.startOperation('Marking brief as failed...');

    try {
      const result = await walde
        .brief({ id: options.briefId })
        .fail({ reason: options.reason, author })
        .resolve();
