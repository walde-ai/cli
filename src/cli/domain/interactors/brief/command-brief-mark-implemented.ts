...
export interface MarkImplementedOptions {
  briefId: string;
  author?: string;
}

export class CommandBriefMarkImplemented {
  constructor(
  ...
  async execute(options: MarkImplementedOptions): Promise<void> {
    ...
    this.presenter.startOperation('Marking brief as implemented...');

    try {
      const result = await walde
        .brief({ id: options.briefId })
        .markImplemented({ author })
        .resolve();
